...
function buildPartyBreakdown(rows) {
  ...
  rows.forEach((row) => {
    ...
    if (row.rank === 1 || row.result === "Leading") {
      current.seatsWon += 1;
    }
...
function extractPartyLinks(summaryHtml, baseUrl) {
  const tables = parseHtmlTables(summaryHtml);
  ...
  if (summaryTable) {
    ...
    for (const row of summaryTable.rows) {
      ...
      const partyCell = cells.find((cell) => /partywise(?:lead|win)result/i.test(cell.href));
      const partyName = partyCell?.text || cells[1]?.text || "";
      const href = partyCell?.href || "";
      ...
      links.push({
        ...
        status: /partywiseleadresult/i.test(href) ? "Leading" : "Winner",
      });
    }
...
function parsePartyPageRows(html, partyLink, region) {
  ...
  return resultsTable.rows
    .slice(1)
    .map((cells, index) => {
      const constituency = cells[headerIndexes.constituency]?.text || "";
...
async function getOfficialEciResults(region) {
  const summaryHtml = await fetchText(region.officialResultsUrl);
  ...
  const settled = await Promise.allSettled(partyLinks.map((partyLink) => fetchText(partyLink.href)));
  const rows = settled.flatMap((result, index) => {
    if (result.status !== "fulfilled") {
      return [];
    }

    return parsePartyPageRows(result.value, partyLinks[index], region);
  });

  const uniqueRows = Array.from(new Map(rows.map((row) => [row.constituency, row])).values());
  const partyBreakdown = buildPartyBreakdown(uniqueRows);
