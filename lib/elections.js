...
async function fetchOfficialResultsText(url) {
  try {
    ...
    if (/access denied/i.test(content)) {
      return fetchText(toMirrorUrl(url), { cache: "no-store" });
    }
...
async function getOfficialEciResults(region) {
  ...
  const settled = await Promise.allSettled(partyLinks.map((partyLink) => fetchOfficialResultsText(partyLink.href)));
